...
class UAVCandidate:
    """
    Group-level UAV candidate.

    A UAV can appear as multiple separated hot thermal components:
        - motors / rotor areas
        - top/bottom body hot spots
        - bright body/frame at distance

    This object groups multiple red component detections into one UAV hypothesis.

    bbox:
        Union bounding box around all grouped components.

    center:
        Center of the group bbox.

    components:
        List of Detection objects inside this group.

    score:
        Group-level score used for ranking.
    """
    bbox: tuple
    center: tuple
    components: list
...
class KalmanTrack:
    ...
    def predict(self):
        ...
        # OpenCV returns a 2D column vector:
        # [[x],
        #  [y],
        #  [vx],
        #  [vy]]
    ...
    def update(self, detection):
        ...
        # OpenCV returns a 2D column vector, so use [row, column].
        self.last_center = (int(corrected[0, 0]), int(corrected[1, 0]))
...
def group_detections_into_uav_candidates(
    detections,
    group_radius=55,
    min_group_components=1,
    max_group_components=8,
    max_group_width=180,
    max_group_height=120,
    group_min_aspect=0.7,
    group_max_aspect=5.5,
    group_min_density=0.005,
    single_component_min_score=130,
    single_component_min_area=5,
):
    """
    Group nearby thermal component detections into UAV-level candidates.

    Important intuition:
        The red boxes are not necessarily whole drones.
        They are hot thermal components: motors, rotor areas, body hot spots, etc.

        A green UAV box should represent a logical group of red components.

    This function is stricter than simple distance grouping.

    It checks:
        1. Component count
        2. Group width and height
        3. Group aspect ratio
        4. Group density / compactness
        5. Single-component strictness

    Single-component groups are allowed because a distant UAV may appear as one blob.
    But single-component groups are treated more strictly to avoid false positives.
    """
    if len(detections) == 0:
        return []
    ...
    for i in range(n):
        ...
        # Breadth-first grouping around detection i.
        # Two detections belong to the same group if their centers are close enough.
        queue = [i]
        visited[i] = True
        group_indices = []
        ...
        components = [detections[k] for k in group_indices]
        num_components = len(components)

        if num_components < min_group_components:
            continue

        if num_components > max_group_components:
            continue

        x1 = min(det.bbox[0] for det in components)
        y1 = min(det.bbox[1] for det in components)
        x2 = max(det.bbox[0] + det.bbox[2] for det in components)
        y2 = max(det.bbox[1] + det.bbox[3] for det in components)
        ...
        # This prevents grouping unrelated forest/ground clutter.
        if w > max_group_width or h > max_group_height:
            continue

        aspect = w / float(h)

        # Reject weird tall/skinny or extremely stretched groups.
        if aspect < group_min_aspect or aspect > group_max_aspect:
            continue

        total_component_area = sum(det.area for det in components)
        group_bbox_area = float(w * h)

        if group_bbox_area <= 0:
            continue

        density = total_component_area / group_bbox_area

        # Reject groups that are too sparse.
        # Sparse groups often mean unrelated background detections got connected.
        if density < group_min_density:
            continue

        total_component_score = sum(det.score for det in components)
        mean_component_score = total_component_score / num_components

        # Single-component groups are risky because many false positives are isolated.
        # Keep them only if they are strong enough.
        if num_components == 1:
            only = components[0]

            if only.score < single_component_min_score:
                continue

            if only.area < single_component_min_area:
                continue
        ...
        # Group score:
        #   - reward component brightness/strength
        #   - reward multiple nearby UAV-like components
        #   - reward compactness
        #   - lightly penalize very large boxes
        component_bonus = 35.0 * num_components
        compactness_bonus = 50.0 * density
        size_penalty = 0.015 * group_bbox_area
...
def select_best_uav_candidate(track, uav_candidates, max_distance=70):
    """
    Select the best UAV group candidate.

    If no track exists:
        choose highest-score group.

    If a track exists:
        prefer groups near the predicted Kalman position.

    This prevents the green box from jumping to random hot background clutter.
    """
    if len(uav_candidates) == 0:
        return None

    if track is None:
        return uav_candidates[0]
    ...
    for cand in uav_candidates:
        ...
        # Penalize distance from predicted track position.
        # A nearby candidate should beat a far high-score clutter group.
        track_distance_penalty = 2.0 * dist

        final_score = cand.score - track_distance_penalty
...
def process_frame_for_detection(frame, args, track):
    ...
    Pipeline:
        ...
        -> clean mask
        -> contour filtering
        -> red thermal component detections
        -> wiser grouping into UAV candidates
        -> Kalman tracking of the selected UAV group

    Returns:
        detections:
            Raw red-box thermal component candidates.

        track:
            Updated Kalman track object.

        track_bbox / track_center:
            Latest green-box tracked UAV group.
    ...
    # Step 1:
    # Detect individual hot thermal components.
    # These are the red boxes.
    ...
    # Step 2:
    # Group nearby hot components into UAV-level candidates.
    # These grouped candidates represent whole-drone hypotheses.
    uav_candidates = group_detections_into_uav_candidates(
        detections,
        group_radius=args.group_radius,
        min_group_components=args.min_group_components,
        max_group_components=args.max_group_components,
        max_group_width=args.max_group_width,
        max_group_height=args.max_group_height,
        group_min_aspect=args.group_min_aspect,
        group_max_aspect=args.group_max_aspect,
        group_min_density=args.group_min_density,
        single_component_min_score=args.single_component_min_score,
        single_component_min_area=args.single_component_min_area,
    )

    # Step 3:
    # Select the best UAV candidate.
    # If a track already exists, candidate selection is track-aware.
    selected = select_best_uav_candidate(
        track,
        uav_candidates,
    ...
    # Step 4:
    # Track the selected group-level UAV candidate.
    if track is None:
        if selected is not None:
            track = KalmanTrack(selected.center)
            track.update(selected)
    else:
        if selected is not None:
            track.update(selected)
        else:
...
def load_config():
    ...
    required_keys = [
        ...
        "max_missed",
        "group_radius",
        "min_group_components",
        "max_group_components",
        "max_group_width",
        "max_group_height",
        "group_radius",
        "min_group_components",
        ...
        "group_max_aspect",
        "group_min_density",
        "single_component_min_score",
        "single_component_min_area",
    ]
