...
def build_frame_source(source_cfg: dict, video_path: Optional[str] = None) -> BaseFrameSource:
    ...
    if src_type in {"live_camera", "opencv_camera"}:
        ...
        return LiveCameraSource(
            device_index=int(source_cfg.get("device_index", source_cfg.get("device", 0))),
            width=int(source_cfg.get("width", 1280)),
            height=int(source_cfg.get("height", 720)),
            fps=float(source_cfg.get("fps", 30.0)),
            fourcc=str(source_cfg.get("fourcc", "MJPG")),
            backend=str(source_cfg.get("backend", "v4l2")),
            source_id=str(source_cfg.get("source_id", "jetson_4k_zoom_camera")),
            max_frames=source_cfg.get("max_frames"),
            warmup_frames=int(source_cfg.get("warmup_frames", 10)),
            frame_stride=stride,
            strict=strict,
        )
